...
def tick(state: EngineState, ik: IKConfig, dt: float) -> StateUpdate:
    ...
    if tip not in state.project.scene.entities:
        # Rate-limit: bad targets at 60Hz would flood journald. Stash the
        # last warn time on EngineState so we don't grow per-tip dicts
        # (clients can spam many different bad targets).
        now = time.monotonic()
        if now - state._last_bad_target_warn_t > 5.0:
            _log.warning("Ovis target %r not in scene; ignoring", tip)
            state._last_bad_target_warn_t = now
        return _build_state_update(state, t, diag=None)
    ...
    result = solve_position_ik(
        ...
        respect_collisions=state.collision_aware,
    ...
    collision_hit = state.collision_aware and not moved and not result.converged
    ...
    if ik.debug and _log.isEnabledFor(logging.DEBUG):
        # One line per non-idle tick. DEBUG level — set log_level=DEBUG in
        # engine.toml (or -v on the CLI) to surface these. Otherwise the
        # firehose is silent even when ik.debug=true so journald isn't
        # buffer-bombed at 60Hz.
        ovis_dump = (
